...
def determine_heading_level(file_path, config, default_level):
    ...
    for custom_path, level in config.get('custom_headings_depths', {}).items():
...
def write_files_to_markdown_with_custom_headings_depths(config):
    index_content = "## Index\n\n"
    file_content = ""
    file_counter = 1
    ...
    for file_type in config['file_extensions']:
        for root, dirs, files in os.walk(config['root_path']):
            dirs[:] = [d for d in dirs if d not in config['exclude_paths']]
            for file in sorted(files):
                if file.endswith(file_type) and not is_excluded(os.path.join(root, file), config['exclude_paths'], config['exclude_files']):
                    file_path = os.path.join(root, file)
                    relative_path = os.path.relpath(file_path, config['root_path'])
                    depth = get_depth(file_path, config['root_path'])
    ...
    with open(config['output_path'], 'w', encoding='utf-8') as md_file:
        md_file.write(full_content)

if __name__ == "__main__":
    config = load_config('config.json')
    write_files_to_markdown_with_custom_headings_depths(config)
    print(f"Markdown document '{config['output_path']}' has been created.")
